...
createProcedure
    : (DELIMITER BIT_OR_OP)? CREATE orReplace? ownerStatement? // here orReplace is MariaDB-specific only
    PROCEDURE fullId '(' procedureParameter? (',' procedureParameter)* ')' routineOption* routineBody (BIT_OR_OP DELIMITER )?
    ;
...
blockStatement
    : (DELIMITER BIT_OR_OP )? (uid ':')? BEGIN NOT? ATOMIC? (
        (declareVariable SEMI)* (declareCondition SEMI)* (declareCursor SEMI)* (
            declareHandler SEMI
        )* procedureSqlStatement*
    )? END uid? (BIT_OR_OP DELIMITER )?
    ;
